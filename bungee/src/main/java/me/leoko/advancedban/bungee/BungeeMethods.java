...
public class BungeeMethods implements MethodInterface {
    ...
    public void loadFiles() {
        try {
            ...
            //Universal.get().immediateBanWords = config.getStringList("ImmediateBanWords"); // Now populated from the redis control proxy via redis
            //Universal.get().warnWords = config.getStringList("WarnWords"); // Now populated from the redis control proxy via redis
    ...
    public void kickPlayer(String player, String reason) {
        if (Universal.isRedis()) {
            BungeeMain.redis.sendChannelMessage("advancedban:main", "kick " + player + " " + reason);
        } else {
    ...
    public void logBan(String name, Punishment punishment) {
        ...
        // LoginEvent, before the player is registered as a ProxiedPlayer.
        ...
        if (Universal.isRedis()) {
            BungeeMain.redis.sendChannelMessage("advancedban:main", "logBan " + name + " " + ip + " " + Universal.get().serialiseObject(punishment));
    ...
    public List<String> getOnlinePlayerNames(boolean redis) {
        ...
        if (Universal.isRedis()) {
            names = BungeeMain.redis.getHumanPlayersOnline()
                    .stream()
    ...
    public void notify(String perm, List<String> notification) {
        if (Universal.isRedis()) {
            notification.forEach((str) -> BungeeMain.redis.sendChannelMessage("advancedban:main", "notification " + perm + " " + str));
        } else {
